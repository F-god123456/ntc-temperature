...
#define KEY_COUNT    5	   					/* 按键个数, 4个独立建 + 3个组合键 */
...
//KEY_RETURN                      
#define KEY_RETURN_GPIO_Port      GPIOB
#define KEY_RETURN_Pin            GPIO_Pin_3
//KEY_ONOFF                       
#define KEY_ONOFF_GPIO_Port       GPIOB
#define KEY_ONOFF_Pin             GPIO_Pin_4  
...
/* 供外部调用的函数声明 */
extern void bsp_InitKey(void);//初始化按键
extern void bsp_KeyScan(void);//扫描所有按键。
extern uint8_t bsp_GetKey(void);//从按键FIFO缓冲区读取一个键值。
extern void checkKeyPresses(void);//检测按键事件并执行相应的操作
#endif
